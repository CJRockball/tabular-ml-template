...
No browser server is started. Data/figure details belong to test_data.py
...
from ml_template.dashboard import app, data
...
def test_configure_artifacts_updates_all_loader_roots(tmp_path: Path) -> None:
    ...
    configured = app.configure_artifacts(root)
...
def test_message_figure_is_safe_empty_state() -> None:
    fig = app._message_figure("No data", "index_monitor.csv not found")
...
def test_pd_notna_handles_registry_null_forms() -> None:
    assert app.pd_notna(0.025)
    assert app.pd_notna("20260911_snapshot")
    assert not app.pd_notna(None)
    assert not app.pd_notna(float("nan"))
    assert not app.pd_notna("NaN")
